...
void CMainFrame::OnTestInput()
{
	...
	if(IDOK == dlg.DoModal()){
		...
		//CString str;
		str.Format(_T("%s\t %s\t \r\n %s\t %4.1f\r\n %s\t %4.1f\r\n %s\t %4.1f\r\n %s\t %4.1f\r\n"),\
			dlg.m_strName,dlg.m_strID,dlg.m_project1,dlg.m_fScore1,\
			dlg.m_project2,dlg.m_fScore2,\
			dlg.m_project3,dlg.m_fScore3,\
			dlg.m_project4,dlg.m_fScore4);
		MessageBox(str,_T("学生录入结果"));	
	}
//————————————————————————————————数据录入文件
	CString filename = _T("e:\\学习\\vc++实验\\vc_experiment\\dc_ex\\test.txt"); //定义文件名
	CFileFind finder;
